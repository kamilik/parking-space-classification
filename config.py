...
class Config:
    ...
    #: Максимум изображений НА КЛАСС после сбора и до разбиения на train/val/test.
    #: Полный PKLot огромен (сотни тысяч изображений), поэтому для обучения за
    #: разумное время берём стратифицированную подвыборку. Итоговый объём ≈
    #: ``MAX_PER_CLASS * 2`` (два класса). ``None`` — без ограничения (весь датасет).
